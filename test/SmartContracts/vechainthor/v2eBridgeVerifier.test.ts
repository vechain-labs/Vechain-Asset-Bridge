    ...
    public async init() {
        if (fs.existsSync(this.configPath)) {
            ...
            try {
                ...
                const vBridgeVerifierPath = path.join(this.contractdir, "/vechainthor/Contract_V2EBridgeVerifier.sol");
                const vBridgeVerifierAbi = JSON.parse(compileContract(vBridgeVerifierPath, 'V2EBridgeVerifier', 'abi',[this.contractdir]));
                const vBridgeVerifierBin = compileContract(vBridgeVerifierPath, 'V2EBridgeVerifier', 'bytecode',[this.contractdir]);
